...
class TestGitLabProvider:
    ...
    def test_get_gitmodules_map_reads_text(self, gitlab_provider, mock_project):
        gitlab_provider.mr = MagicMock(target_branch="main", source_branch="feature")
        mock_file = MagicMock(ProjectFile)
        mock_file.decode.return_value = b"[submodule \"lib\"]\n\tpath = lib\n\turl = https://example.com/lib.git\n"
        mock_project.files.get.return_value = mock_file

        result = gitlab_provider._get_gitmodules_map()

        assert result == {"lib": "https://example.com/lib.git"}

    def test_get_gitmodules_map_decode_failure_returns_empty(self, gitlab_provider, mock_project):
        gitlab_provider.mr = MagicMock(target_branch="main", source_branch="feature")
        mock_file = MagicMock(ProjectFile)
        mock_file.decode.side_effect = Exception("boom")
        mock_file.content = "W3submoduleIFwibGliXCI+XG4JcGF0aCA9IGxpYlxuCXVybCA9IGh0dHBzOi8vZXhhbXBsZS5jb20vbGliLmdpdFxu"
        mock_project.files.get.return_value = mock_file
